...
#define LUMA_DECODER_FIR_SIZE  60
...
void main() {
    ...
    for (int d = -LUMA_DECODER_FIR_SIZE; d < 0; d++) {
        vec2 pos = vec2(frag_coord.x + float(d), frag_coord.y);

        vec3 s = texture(input_texture, pos / resolution.xy).rgb;
        
        float filt = lowpass(0.09, float(d));

        yiq += s * vec3(float(LUMA_DECODER_FIR_SIZE) * 3.5, 0.0, 0.0) * filt;
    ...
    for (int d = 0; d < CHROMA_DECODER_FIR_SIZE; d++) {
        vec2 pos = vec2(frag_coord.x + float(d) - float(CHROMA_DECODER_FIR_SIZE / 2), frag_coord.y);

        vec3 s = texture(input_texture, pos / resolution.xy).rgb;

        float t = fc * (frag_coord.x + float(d));
        ...
        float filt = bandpass(0.999, 1.0, float(d), float(CHROMA_DECODER_FIR_SIZE));
    ...
    frag_color = vec4((yiq_to_rgb * yiq), 1.0);
}